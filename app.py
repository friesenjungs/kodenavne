...
def join_room_socket(data):
    ...
    if len(search_session) == 1:
        ...
        emit("show toast", {"title": "New player", "message": f"Let's welcome {game_session.user_name} in the room"},
    ...
    if "team" in data and "role" in data and isinstance(data["team"], int) and isinstance(data["role"], str):
        ...
        if 0 < data["team"] <= amount_of_teams and chosen_role is not None:
            ...
            if not already_assigned:
                if game_session.role_id is not None:
                    role_room = game_session.role.role_name
                    leave_room(game_session.game.room_code+role_room)
                    print(f"User with cookie {session_cookie} left room {game_session.game.room_code}/{role_room}")
                new_role_room = chosen_role.role_name
                join_room(f"{game_session.game.room_code}/{new_role_room}")
                print(f"User with cookie {session_cookie} joined room {game_session.game.room_code}/{new_role_room}")
...
def start_game_socket():
    ...
    if "game_session_ids" in session:
        ...
        if sum(team_sizes) != len(active_users_in_session):
            reason += "- All active players muss be assigned to a team"
            ...
            if game_session.admin:
                ...
                wordbank = db.WordBank.query.filter_by(wordbank_name="base").first()
                words = wordbank.words.order_by(func.random()).limit(5).all()
                ...
                for i, word in enumerate(words):
                    gamewords.append(db.GameWord(gameset_id=gameset.gameset_id, word_id=word.word_id, card_position=i, cardrole_id=1))
                ...
                emit("start game", room=game_instance.room_code)
    ...
    emit("show toast", {"title": "Game error", "message": f"{reason}"})
    return False
...
def disconnect_socket():
    ...
    if "game_session_ids" in session:
        ...
        emit("show toast", {"title": "Player disconnected", "message": f"Oh no! {game_session.user_name} left the room"})
        ...
        if len(remaining_players) == 0:
            ...
            emit("show toast", {"title": "New admin", "message": f"Player {new_admin.user_name} is the new admin"})
